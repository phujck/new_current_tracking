...
if Tracking:
    ...
    newparameternames = '-%s-nsites-%s-cycles-%s-U-%s-t-%s-n-%s-delta-%s-field-%s-amplitude-%s-ascale-%s-scalefactor.npy' % (
        nx, cycles, U_track, t, number, delta, field, F0, ascale, scalefactor)

    J_field_track = np.load('./data/tracking/Jfieldspecial' + newparameternames) / scalefactor
    ...
    phi_track = np.load('./data/tracking/phispecial' + newparameternames)

    newparameternames2 = '-%s-nsites-%s-cycles-%s-U-%s-t-%s-n-%s-delta-%s-field-%s-amplitude-%s-ascale-%s-scalefactor.npy' % (
        nx, cycles, U_track2, t, number, delta, field, F0, ascale2, scalefactor2)

    J_field_track2 = np.load('./data/tracking/Jfieldspecial' + newparameternames2) / scalefactor2
    t_track2 = np.linspace(0.0, cycles, len(J_field_track2))
    phi_track2 = np.load('./data/tracking/phispecial' + newparameternames2)
...
ref_J = np.load('./data/original/Jfield' + zeroparameternames)
ref_phi = np.load('./data/original/phi' + zeroparameternames)
times = np.linspace(0.0, cycles, len(ref_J))

plt.figure(figsize=(16 * 1, 9 * 1.4))
plt.subplot(211)
if Tracking:
    plt.plot(t_track, phi_track / scalefactor,
             label='$\\frac{U}{t_0}=$%s' % (prop_track.U), color='blue')
    plt.plot(t_track2, phi_track2 / scalefactor2,
             label='$\\frac{U}{t_0}=$%s' % (prop_track2.U), color='red', linestyle='-.')
...
plt.semilogy(w, spec, linestyle='dashed', label='Original', color='black')

exact = np.gradient(J_field_track2, delta_track)
w, spec = har_spec.spectrum_welch(exact, delta1)
w *= 2. * np.pi / prop.field
plt.semilogy(w, spec, label='Boosted', color='blue')

exact = np.gradient(J_field_track, delta_track)
w, spec = har_spec.spectrum_welch(exact, delta1)
w *= 2. * np.pi / prop.field
plt.semilogy(w, spec, color='red', linestyle='-.')
...
for j in range(len(ascales)):
    ...
    norms.append(np.linalg.norm(ref_J - J_field_track, ord=2))
    exact = np.gradient(J_field_track, delta_track)
    w, spec = har_spec.spectrum_welch(exact, delta1)
    w *= 2. * np.pi / prop.field
